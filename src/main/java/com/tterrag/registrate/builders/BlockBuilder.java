...
public class BlockBuilder<T extends Block, P> {
    ...
    public BlockBuilder(CreateRegistrate owner, String name, NonNullFunction<BlockBehaviour.Properties, T> factory) {
        this.owner = owner;
        this.name = name;
    ...
    public BlockBuilder<T, P> tag(TagKey<Block> tag) {
        return this;
    }
    ...
    public <I extends BlockItem> ItemBuilder<I, BlockBuilder<T, P>> item(java.util.function.BiFunction<T, Item.Properties, I> factory) {
        itemBuilder = new ItemBuilder<>(owner, name, props -> factory.apply((T) owner.blockByName(name), props), this);
        return (ItemBuilder<I, BlockBuilder<T, P>>) itemBuilder;
    }
    ...
    public BlockEntry<T> register() {
        ...
        BlockBehaviour.Properties props = properties.apply(BlockBehaviour.Properties.of())
                .setId(key);
        T block = factory.apply(props);
        owner.rememberBlock(name, block);
        ...
        if (simpleItem || itemBuilder != null) {
            ...
            Item item = itemBuilder == null ? new BlockItem(block, itemProps) : itemBuilder.create(itemProps);
            Registry.register(BuiltInRegistries.ITEM, id, item);
            owner.trackItem(new ItemEntry<>(name, item));
        }
        onRegister.accept(block);
        BlockEntry<T> entry = new BlockEntry<>(name, block);
        owner.trackBlock(entry);
        return entry;
    }

    private Block getRegisteredBlock() {
        return owner.blockByName(name);
    }
}
